    ...
    let sum: i64 = chars
        ...
        .filter(|ch| {
            numbers
                .clone()
                .into_iter()
                .filter(|nr| nr.intesects(ch.position, ch.line_number))
                .count()
                .eq(&2)
        })
        .map(|ch| {
            ...
            nums.clone().into_iter().next().unwrap().value
                * nums.clone().into_iter().last().unwrap().value
        })
        .sum();

    println!("{:?}", chars.clone().into_iter().count());
    println!("{:?}", sum)
}
